...
class ControllerExecutor(object):
    ...
    def __init__(self, ltl_filename, synthesis_options, run_executor, init_state_dict={}):
        ...
        rospy.Subscriber('input_manager/incoming_inputs', controller_executor.msg.stringKeyBoolValueDict, callback=self.update_inputs, queue_size=1, buff_size=70000)
    ...
    def update_inputs(self, data):
        ...
        #controller_logger.error("!!1111incoming inputs"+str(self._incoming_inputs))#chuanwei
        #controller_logger.error("!!2222dataaaa"+str(data))#chuanwei
        self._incoming_inputs = dict(zip(data.keys, data.values))
        #controller_logger.error("!!3333incoming inputs"+str(self._incoming_inputs))#chuanwei
	...
	#else:
            #controller_logger.info("same output")	
